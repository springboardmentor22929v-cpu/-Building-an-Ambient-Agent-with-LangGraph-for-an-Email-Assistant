...
from email_assistant.db import get_connection
...
def get_memory(store, namespace, default_content=None):
    ...
    cursor.execute(
        ...
        (namespace_str, "user_preferences")
    )
    row = cursor.fetchone()

    if row:
        store.put(namespace, "user_preferences", row[0])   # 🔥 SYNC LANGGRAPH MEMORY
        conn.close()
        return row[0]
    # ---------------------------------------------
...
def update_memory(store, namespace, messages):
    ...
    # ---------------- SAFE DEFAULT ----------------
    if "triage_preferences" in namespace:
        default_content = default_triage_instructions
    elif "cal_preferences" in namespace:
        default_content = default_cal_preferences
    else:
        default_content = default_response_preferences

    # SAFE FETCH from LangGraph store
    current_profile = get_memory(store, namespace, default_content)
    ...
    cursor.execute(
        ...
        """,
        (namespace_str, "user_preferences", result.user_preferences)
    )
...
def interrupt_handler(state: State, store: BaseStore) -> Command[Literal["llm_call", "__end__"]]:
    ...
    for tool_call in state["messages"][-1].tool_calls:
        ...
        if response["type"] == "accept":
            ...
            if isinstance(response["args"], dict) and "args" in response["args"]:
                edited_args = response["args"]["args"]
            else:
                edited_args = response["args"]

            edited_args = {**tool_call["args"], **edited_args}
