...
Return only the classification label. Do not include explanations or extra text.
</ Instructions >
...
- If an email mentions a date, time, meeting, or availability:
  you MUST reason about scheduling.
- If a specific time window is mentioned:
  you MUST call check_calendar_availability before replying.
- If the time is available:
  you MUST schedule the meeting before replying.
- Never schedule without checking availability first.
"""

# Default triage instructions 
default_triage_instructions ="""Classify the email into exactly ONE of the following categories:

- ignore: Low-value emails such as spam, promotions, newsletters, or
  informational messages that require no action.

- notify: Important or sensitive emails that do not require a direct
  response but should be reviewed by a human.

- respond: Clear, safe, and actionable emails that require a reply.
